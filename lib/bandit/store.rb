    ...
    class SQLiteDB < Abstract
      ...
      def ban ip, jail
        ...
        row = db.get_first_row <<~SQL, {ip:, jail:, offset:}
          ...
          ON CONFLICT DO UPDATE SET
            ...
            updated_at = datetime('now')
            WHERE allowed != 1
              AND 86400.0*(julianday('now') - julianday(updated_at)) > 1
      ...
      def unban ip
        row = db.get_first_row <<~SQL, [ip]
          UPDATE bans
          SET unban_at = NULL, updated_at = datetime('now')
          WHERE ip = ?
      ...
      def expired
        db.execute(<<~SQL).flat_map(&:values)
          ...
          WHERE NOT allowed
            AND unban_at
            AND julianday(unban_at) < julianday('now')
        SQL
      end
      ...
      def next_unban
        db.get_first_value(<<~SQL)
          SELECT
            86400.0 * (                         -- next non-null unban_at time
              julianday(unban_at) -
              julianday('now')) AS t
            FROM bans
          ...
          UNION                                 -- OR:
          SELECT
            86400.0 * (                         -- next updated_at + 3+count weeks
      ...
      def report
        system <<~"EOF"
            sqlite3 -column -header #{path} <<-SQL | sed -re 's/ +$//'
              SELECT bans.ip as ip, jail, count,
                     datetime(unban_at, 'localtime') as local_unban_at,
                     dns.name
                FROM bans
